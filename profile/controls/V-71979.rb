...
command:# grep localpkg_gpgcheck /etc/yum.conflocalpkg_gpgcheck=1If
\"localpkg_gpgcheck\" is not set to \"1\", or if options are missing or
commented out, ask the System Administrator how the signatures of local
packages and other operating system components are verified. If there is no
process to validate the signatures of local packages that is approved by the
...
\"/etc/yum.conf\" file:localpkg_gpgcheck=1"
end
