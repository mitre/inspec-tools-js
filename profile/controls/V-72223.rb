...
resources committed by the managed network element. Terminating network
...
inactivity.Check the value of the system inactivity timeout with the following
command:# grep -i tmout /etc/bashrc /etc/profile.d/*TMOUT=600If \"TMOUT\" is
not set to \"600\" or less in \"/etc/bashrc\" or in a script created to enforce
session termination after inactivity, this is a finding."
...
or after a period of inactivity.Add or update the following lines in
\"/etc/profile\".TMOUT=600readonly TMOUTexport TMOUTOr create a script to
enforce the inactivity timeout (for example /etc/profile.d/tmout.sh) such
as:#!/bin/bashTMOUT=600readonly TMOUTexport TMOUT"
end
