...
successful/unsuccessful attempts to use the \"chsh\" command occur.Check for
...
/usr/bin/chsh /etc/audit/audit.rules-a always,exit -F path=/usr/bin/chsh -F
...
successful/unsuccessful attempts to use the \"chsh\" command occur.Add or
update the following rule in \"/etc/audit/rules.d/audit.rules\": -a always,exit
-F path=/usr/bin/chsh -F perm=x -F auid>=1000 -F auid!=4294967295 -k
