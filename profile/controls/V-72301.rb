...
"CCI-001814"]
  ...
  tag "ia_controls": nil
  tag "check": "Verify a TFTP server has not been installed on the system.Check
to see if a TFTP server has been installed with the following command:# yum
list installed tftp-servertftp-server-0.49-9.el7.x86_64.rpmIf TFTP is installed
and the requirement for TFTP is not documented with the ISSO, this is a
finding."
  tag "fix": "Remove the TFTP package from the system with the following
command:# yum remove tftp"
end
